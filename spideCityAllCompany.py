...
def search(driver, city, isdebug, web_order):
    ...
    for i in range(1, page_num + 1):
        page_url.append(
            "http://m.54114.cn/" + city + "/" + web_order + "_p" + str(i) + "/")  # -------------------网址修改之二
    ...
    for j in range(url_num):  # 对每个公司的链接，进入该网址，获取信息
        ...
        try:  # 第1次获取
            ...
            if (mail_or_url.find(u'邮箱') == 0):
                have_mail = 1
                mails = re.findall(r"[a-zA-Z0-9\.\-+_]+@?[a-zA-Z0-9\.\-+_]+[\.]?[a-zA-Z]+",
                                   mail_or_url)  # 邮箱不规范，@后没写点，因此这里的.设置为可选项
                if (isdebug == 1):
                    print(mails[0])
        ...
        try:  # 第2次获取
            ...
            if (have_url == 0):
                ...
                pass
            if (have_addr == 0):
                if (url_or_addr.find(u'网址：') == -1 and url_or_addr.find(
                        u'地址：') == 0):  # 有些情况下，网址的那一行里有“下载地址”。 首字符匹配到“地址”并且没有出现“网址”才能算作地址
                    if (have_url == 0):
                        worksheet.write(j + 1, 4, label='null')  # url is null
                    have_addr = 1
                    if (isdebug == 1):
